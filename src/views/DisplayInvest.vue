      ...
      <section class="section-container metrics-section">
        ...
        <div class="metrics-grid">
          <div class="metric-item" v-for="(metric, index) in propertyData?.metrics" :key="index">
            ...
            <span class="metric-value" :class="{ 'primary-text': metric.isPrimary }">{{ metric.value }}</span>
          </div>
        </div>
...
function usePropertyData(endpoint) {
  const defaultData = {
    images: [], status: {}, specs: {}, overview: '', options: { directBuy: {} }, metrics: []
  };
...
/* Investment Unique CSS */
.property-detail-main { padding-bottom: 2rem; }
...
.investment-option-details h3 { font-size: 1.1rem; font-weight: 600; margin-bottom: 0.5rem; }
.option-description { font-size: 0.9rem; color: var(--muted-foreground); margin-bottom: 1rem; }
